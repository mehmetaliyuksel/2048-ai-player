...
public class ExpectiMax {
    ...
    public Node expectiMax(Node node, boolean isMaxNode, int depth, int ply) {
        ...
        if (ply * 2 == depth + 1) {
            double val = node.getChildren()
                    .stream()
                    .mapToDouble(child -> child.getProbability() * heuristic1(node.getState().getTiles()))
                    .sum();
            node.setValue(val);
        ...
        if (isMaxNode) {
            ...
            node.setValue(node.getChildren()
                    ...
                    .mapToDouble(child -> {
                        ...
                        double val = child.getProbability() * newNode.getValue();
                        if (val == Double.POSITIVE_INFINITY || val == Double.NEGATIVE_INFINITY)
                            System.out.println();
                        return val;
                    })
    ...
    // System.out.println(
    // "DEPTH: " + depth + "-- RIGHT : newUtil ----------" + newUtility + "maxUtil
    // :" + maxUtility);
    ...
    // double possibility = (isNewlyAddedTileIs2) ? 0.9 * 1 / numOfAvailableSpace :
    // 0.1 * 1 / numOfAvailableSpace;
    ...
    // public double evaluate(Tile[] state, boolean isNewlyAddedTileIs2, int
    // numOfAvailableSpace) {
    ...
    // Game2048Util.addOneTile(availableSpacesOnBoard.get(i)),
